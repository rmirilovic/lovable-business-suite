...
export interface ReprocessingDeliveryNote {
  id: string;
  company_id: string;
...
export interface ReprocessingDeliveryNoteItem {
  id: string;
...
export const RDN_STATUS_LABELS: Record<string, string> = {
  draft: "Nacrt",
  posted: "Proknjižena",
};

export const RDN_STATUS_COLORS: Record<string, string> = {
  draft: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400",
  posted: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400",
};

export function useReprocessingDeliveryNotes() {
  ...
  const notesQuery = useQuery({
    queryKey: ["reprocessing-delivery-notes", companyId, yearId],
    queryFn: async () => {
      if (!companyId || !yearId) return [];
      const { data, error } = await (supabase as any)
        .from("reprocessing_delivery_notes")
        .select(`*, warehouse:warehouses(id, code, name), work_order:reprocessing_work_orders(id, order_number)`)
        .eq("company_id", companyId)
        .eq("business_year_id", yearId)
        .order("delivery_number", { ascending: false });
      if (error) throw error;
      return data as ReprocessingDeliveryNote[];
    },
  ...
  const createNote = useMutation({
    mutationFn: async (formData: {
      ...
      // Find existing delivery notes for suffix
      ...
      // Auto-insert items from work order output items
      const { data: woItems } = await (supabase as any)
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reprocessing-delivery-notes"] });
  ...
  const postNote = useMutation({
    mutationFn: async (id: string) => {
      const { data, error } = await (supabase as any).rpc("post_reprocessing_delivery_note", {
        _note_id: id,
        _user_id: user?.id,
      });
      if (error) throw error;
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reprocessing-delivery-notes"] });
      queryClient.invalidateQueries({ queryKey: ["reprocessing-delivery-note"] });
  ...
  const unpostNote = useMutation({
    mutationFn: async (id: string) => {
      const { data, error } = await (supabase as any).rpc("unpost_reprocessing_delivery_note", {
        _note_id: id,
        _user_id: user?.id,
      });
      if (error) throw error;
...
export function useReprocessingDeliveryNote(id: string | undefined) {
  return useQuery({
    queryKey: ["reprocessing-delivery-note", id],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("reprocessing_delivery_notes")
        .select(`*, warehouse:warehouses(id, code, name), work_order:reprocessing_work_orders(id, order_number)`)
...
export function useReprocessingDeliveryNoteItems(noteId: string | undefined) {
  const queryClient = useQueryClient();
  const query = useQuery({
    queryKey: ["reprocessing-delivery-note-items", noteId],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("reprocessing_delivery_note_items")
        .select("*")
        ...
        .order("item_order");
      if (error) throw error;
      return data as ReprocessingDeliveryNoteItem[];
    },
    enabled: !!noteId,
  });
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["reprocessing-delivery-note-items", noteId] });
