...
export function exportRequisitionsToExcel(items: MaterialRequisition[], meta: ExportMeta) {
  ...
  ws["!cols"] = [
    { wch: 12 }, { wch: 12 }, { wch: 25 }, { wch: 14 }, { wch: 20 }, { wch: 20 }, { wch: 14 },
  ];
...
async function buildPdf(items: MaterialRequisition[], meta: ExportMeta): Promise<jsPDF> {
  ...
  const head = [["Broj", "Datum", "Magacin", "Radni nalog", "Izdao", "Primio", "Status"]];
  ...
  autoTable(doc, {
    ...
    columnStyles: { 0: { cellWidth: 22 }, 1: { cellWidth: 22 }, 6: { cellWidth: 22 } },
  });
