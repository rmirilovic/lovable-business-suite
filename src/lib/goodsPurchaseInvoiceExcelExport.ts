...
export function exportGoodsPurchaseInvoiceToExcel(
  ...
  const headerData = [
    ...
    ["Magacin", invoice.warehouse ? `${invoice.warehouse.code} - ${invoice.warehouse.name}` : ""],
    [],
  ...
  XLSX.writeFile(wb, `UFR_${invoice.internal_number.replace(/\//g, "-")}.xlsx`);
}
