...
import { ServicePurchaseInvoice } from "@/hooks/useServicePurchaseInvoices";
...
function mapRows(items: ServicePurchaseInvoice[]) {
...
export function exportServicePurchaseInvoicesToExcel(items: ServicePurchaseInvoice[], meta: ExportMeta) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "UF Usluge");
  XLSX.writeFile(wb, "ulazne_fakture_usluge.xlsx");
}

async function buildPdf(items: ServicePurchaseInvoice[], meta: ExportMeta): Promise<jsPDF> {
  ...
  y += 8;
  doc.setFontSize(14);
  doc.text("Ulazne fakture za usluge", pageWidth / 2, y, { align: "center" });
  y += 6;
...
export async function exportServicePurchaseInvoicesToPdf(items: ServicePurchaseInvoice[], meta: ExportMeta) {
  const doc = await buildPdf(items, meta);
  doc.save("ulazne_fakture_usluge.pdf");
}

export async function printServicePurchaseInvoices(items: ServicePurchaseInvoice[], meta: ExportMeta) {
  const doc = await buildPdf(items, meta);
