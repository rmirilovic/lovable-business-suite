...
import { AdvancePurchaseInvoice } from "@/hooks/useAdvancePurchaseInvoices";
...
function mapRows(items: AdvancePurchaseInvoice[]) {
  return items.map((inv) => ({
    ...
    "PDV obveznik": inv.supplier_is_in_pdv ? "Da" : "Ne",
...
export function exportAdvancePurchaseInvoicesToExcel(items: AdvancePurchaseInvoice[], meta: ExportMeta) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "UFA");
  XLSX.writeFile(wb, "ulazne_fakture_avansi.xlsx");
}

async function buildPdf(items: AdvancePurchaseInvoice[], meta: ExportMeta): Promise<jsPDF> {
  ...
  y += 8;
  doc.setFontSize(14);
  doc.text("Ulazne fakture za plaćene avanse", pageWidth / 2, y, { align: "center" });
  y += 6;
...
export async function exportAdvancePurchaseInvoicesToPdf(items: AdvancePurchaseInvoice[], meta: ExportMeta) {
  const doc = await buildPdf(items, meta);
  doc.save("ulazne_fakture_avansi.pdf");
}

export async function printAdvancePurchaseInvoices(items: AdvancePurchaseInvoice[], meta: ExportMeta) {
  const doc = await buildPdf(items, meta);
