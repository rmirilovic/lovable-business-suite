...
export function exportBilansStanjaToExcel(rows: ComputedRow[], meta: ExportMeta) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Bilans stanja");
  XLSX.writeFile(wb, `bilans_stanja_${meta.yearLabel}.xlsx`);
}
...
async function buildPdf(rows: ComputedRow[], meta: ExportMeta): Promise<jsPDF> {
  ...
  doc.setFontSize(14);
  doc.text("BILANS STANJA", pw / 2, y, { align: "center" });
  y += 6;
  doc.setFontSize(9);
...
export async function exportBilansStanjaToPdf(rows: ComputedRow[], meta: ExportMeta) {
  const doc = await buildPdf(rows, meta);
  doc.save(`bilans_stanja_${meta.yearLabel}.pdf`);
}

export async function printBilansStanja(rows: ComputedRow[], meta: ExportMeta) {
  const doc = await buildPdf(rows, meta);
