...
// Calculate net price (price minus discount)
const calculateNetPrice = (unitPrice: number, discountPercent: number): number => {
  return unitPrice * (1 - discountPercent / 100);
};

export async function generateGoodsPurchaseInvoicePdf(
  invoice: GoodsPurchaseInvoice,
  ...
  doc.setFontSize(16);
  doc.setFont("Roboto", "bold");
  doc.text("ULAZNA FAKTURA ZA ROBU", pageWidth / 2, yPos, { align: "center" });
  yPos += 7;
  ...
  if (invoice.warehouse) {
    doc.text(`Magacin: ${invoice.warehouse.code} - ${invoice.warehouse.name}`, 14, yPos);
    yPos += 4;
  }
  ...
  const tableData = items.map((item, index) => [
    ...
    item.discount_percent > 0 ? `${formatPdfNumber(item.discount_percent)}%` : "-",
    formatPdfNumber(calculateNetPrice(item.unit_price, item.discount_percent)),
  ...
  if (options?.print) {
    ...
    doc.save(
      `Ulazna_faktura_roba_${invoice.internal_number.replace(/\//g, "-")}.pdf`
    );
  }
}
