...
export function ArticleReprocessingDeliveryNotesDialog({ open, onOpenChange, articleId, articleCode, articleName }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: items, error } = await (supabase as any)
        ...
        .select("delivery_note_id, qty_total, delivered_kg, unit_price, item_value")
  ...
  const totals = rows.reduce(
    ...
    { qty_total: 0, delivered_kg: 0, item_value: 0 }
  );
