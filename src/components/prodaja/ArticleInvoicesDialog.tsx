...
export function ArticleInvoicesDialog({ open, onOpenChange, articleId, articleCode, articleName }: Props) {
  const { selectedCompany, selectedYear } = useAuth();
  const [rows, setRows] = useState<ArticleInvoiceRow[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        let query = supabase
          ...
          .select(`
            ...
            invoice:invoices!inner(
              invoice_date, invoice_number, status,
        ...
        const mapped: ArticleInvoiceRow[] = (data || []).map((item: any) => ({
          invoice_date: item.invoice?.invoice_date ?? "",
          invoice_number: item.invoice?.invoice_number ?? "",
          partner_code: item.invoice?.partner?.code ?? "",
          partner_name: item.invoice?.partner_name ?? item.invoice?.partner?.name ?? "",
        ...
        mapped.sort((a, b) => a.invoice_date.localeCompare(b.invoice_date) || a.invoice_number.localeCompare(b.invoice_number));
        setRows(mapped);
      } catch (err) {
        console.error("Error fetching article invoices:", err);
        setRows([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>
            Artikal na fakturama: {articleCode} – {articleName}
          </DialogTitle>
        </DialogHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[120px]">Broj fakture</TableHead>
                <TableHead className="min-w-[200px]">Kupac</TableHead>
                <TableHead className="text-right w-[100px]">Količina</TableHead>
                <TableHead className="text-right w-[120px]">Fakturisana cena</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                    Nema faktura za ovaj artikal u izabranom periodu.
                  </TableCell>
                </TableRow>
              ) : (
                <>
