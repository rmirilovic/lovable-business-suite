...
export default function ReprocessingDeliveryNotesList() {
  const navigate = useNavigate();
  const { selectedCompany, selectedYear, user } = useAuth();
  const companyId = selectedCompany?.id;
  const { notes, isLoading, createNote, deleteNote, postNote, unpostNote } = useReprocessingDeliveryNotes();
  ...
  const [newForm, setNewForm] = useState({ delivery_date: format(new Date(), "yyyy-MM-dd"), warehouse_id: "", work_order_id: "", production_line: 1, responsible_person: "" });

  useEffect(() => {
    if (!user?.id) return;
    supabase.from("profiles").select("first_name, last_name").eq("id", user.id).single()
      .then(({ data }) => { if (data) setOperatorName([data.first_name, data.last_name].filter(Boolean).join(" ")); });
  }, [user?.id]);

  // Item summaries
  const noteIds = useMemo(() => notes.map(n => n.id), [notes]);
  const { data: itemSummary } = useQuery({
    queryKey: ["rdn-items-summary", companyId, noteIds],
    queryFn: async () => {
      ...
      for (const item of (data || []).sort((a: any, b: any) => (a.item_order ?? 0) - (b.item_order ?? 0))) {
        if (!map[item.delivery_note_id]) map[item.delivery_note_id] = { totalValue: 0, totalKg: 0, firstArticle: "" };
        ...
        if (!map[item.delivery_note_id].firstArticle) map[item.delivery_note_id].firstArticle = `${item.article_code} - ${item.article_name}`;
      }
      return map;
    },
  ...
  const filtered = notes.filter((n) => {
    ...
    const matchSearch = !search || n.delivery_number.includes(search) || n.work_order?.order_number?.includes(search) || (itemSummary?.[n.id]?.firstArticle ?? "").toLowerCase().includes(s);
  ...
  const persistAndNavigate = (id: string) => {
    saveFilters({ search, statusFilter, dateFrom, dateTo, sortColumn, sortDirection });
    navigate(`/proizvodnja/predajnice-prerada/${id}`);
  };
  ...
  return (
    <MainLayout title="Predajnice GP po RN za preradu">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between">
            <div className="flex gap-4 flex-1 items-end">
              <div className="relative flex-1 max-w-xs">
                ...
                <Input placeholder="Broj, RN ili proizvod..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" autoComplete="off" />
              </div>
              <div className="flex gap-1">
                {["all", "draft", "posted"].map((s) => (
                  <Button key={s} variant={statusFilter === s ? "default" : "outline"} size="sm" onClick={() => setStatusFilter(s)}>
                    {s === "all" ? "Svi" : RDN_STATUS_LABELS[s]}
                  </Button>
                ))}
              </div>
            </div>
        ...
        <TableScrollContainer ref={scrollRef} className="flex-1">
          <Table>
            <TableHeader><TableRow>
              ...
              <TableHead className="w-[90px]"><SortableHeader column="work_order" label="RN" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} /></TableHead>
              ...
              <TableHead><SortableHeader column="article" label="Gotov proizvod" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} /></TableHead>
              <TableHead className="w-[110px] text-right"><SortableHeader column="total_kg" label="Ukupno kg" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} className="justify-end" /></TableHead>
              <TableHead className="w-[110px] text-right"><SortableHeader column="total_value" label="Vrednost" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} className="justify-end" /></TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Nema predajnica.</TableCell></TableRow>
              ) : sorted.map((note) => (
                <TableRow key={note.id} className="cursor-pointer hover:bg-muted/50" onClick={() => persistAndNavigate(note.id)}>
                  ...
                  <TableCell className="text-xs truncate max-w-[220px]">{itemSummary?.[note.id]?.firstArticle ?? "-"}</TableCell>
                  ...
                  <TableCell><Badge className={cn("text-xs", RDN_STATUS_COLORS[note.status])}>{RDN_STATUS_LABELS[note.status] ?? note.status}</Badge></TableCell>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8"><MoreHorizontal className="w-4 h-4" /></Button></DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => persistAndNavigate(note.id)}><Eye className="w-4 h-4 mr-2" /> Otvori</DropdownMenuItem>
                        {note.status === "draft" && <DropdownMenuItem onClick={() => { if (confirm(`Proknjižiti ${note.delivery_number}?`)) postNote.mutateAsync(note.id); }}><Lock className="w-4 h-4 mr-2" /> Proknjiži</DropdownMenuItem>}
                        {note.status === "posted" && <DropdownMenuItem className="text-destructive" onClick={() => { if (confirm(`Poništiti knjiženje ${note.delivery_number}?`)) unpostNote.mutateAsync(note.id); }}><Unlock className="w-4 h-4 mr-2" /> Poništi knjiženje</DropdownMenuItem>}
                        {note.status === "draft" && <DropdownMenuItem onClick={() => { if (confirm(`Obrisati ${note.delivery_number}?`)) deleteNote.mutateAsync(note.id); }} className="text-destructive"><Trash2 className="w-4 h-4 mr-2" /> Obriši</DropdownMenuItem>}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
      ...
      <Dialog open={showNewDialog} onOpenChange={setShowNewDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>Nova predajnica GP po RN za preradu</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-1"><Label>Datum</Label><LocaleDateInput value={newForm.delivery_date} onChange={(v) => setNewForm((p) => ({ ...p, delivery_date: v }))} /></div>
            <div className="space-y-1">
              <Label>Radni nalog za preradu</Label>
              <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={newForm.work_order_id} onChange={(e) => setNewForm((p) => ({ ...p, work_order_id: e.target.value }))}>
                <option value="">-- Izaberite RN --</option>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!newForm.warehouse_id || !newForm.work_order_id || createNote.isPending}>Kreiraj</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </MainLayout>
