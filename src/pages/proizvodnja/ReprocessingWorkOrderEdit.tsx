...
import { ArrowLeft, Plus, Trash2, Rocket, Lock, Save, Undo2 } from "lucide-react";
import {
              ...
              <TableBody>
                {outputItems.length === 0 ? (
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right"><LocaleNumberInput value={item.unit_price} onValueChange={(v) => handleUpdateOutput(item, "unit_price", v)} disabled={!isDraft} className="w-[90px] text-right h-8" /></TableCell>
                    <TableCell className="text-right"><LocaleNumberInput value={item.launched_qty} onValueChange={(v) => handleUpdateOutput(item, "launched_qty", v)} disabled={!isDraft} className="w-[90px] text-right h-8" /></TableCell>
              ...
              <TableBody>
                {inputItems.length === 0 ? (
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.article_name}</TableCell>
                    <TableCell>{item.unit}</TableCell>
                    <TableCell className="text-right"><LocaleNumberInput value={item.quantity} onValueChange={(v) => handleUpdateInput(item, "quantity", v)} disabled={isClosed} className="w-[90px] text-right h-8" /></TableCell>
                    <TableCell className="text-right"><LocaleNumberInput value={item.unit_price} onValueChange={(v) => handleUpdateInput(item, "unit_price", v)} disabled={isClosed} className="w-[90px] text-right h-8" /></TableCell>
              ...
              <TableBody>
                {materials.length === 0 ? (
                  ...
                  <TableRow key={mat.id}>
                    ...
                    <TableCell className="text-right"><LocaleNumberInput value={mat.quantity} onValueChange={(v) => handleUpdateMaterial(mat, "quantity", v)} disabled={isClosed} className="w-[90px] text-right h-8" /></TableCell>
                    <TableCell className="text-right"><LocaleNumberInput value={mat.unit_price} onValueChange={(v) => handleUpdateMaterial(mat, "unit_price", v)} disabled={isClosed} className="w-[90px] text-right h-8" /></TableCell>
