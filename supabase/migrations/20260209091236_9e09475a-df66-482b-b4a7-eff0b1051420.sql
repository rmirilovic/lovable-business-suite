...
-- Add partner_id to calculation_additional_costs for tracking cost suppliers
ALTER TABLE public.calculation_additional_costs
ADD COLUMN partner_id uuid REFERENCES public.partners(id);

-- Create posting function for purchase price calculations
CREATE OR REPLACE FUNCTION public.post_purchase_price_calculation(
...
BEGIN
  ...
  AND svk = '1'; -- Only goods (SVK=1) have markup
  ...
  IF v_total_additional_costs > 0 OR v_total_markup > 0 THEN
    ...
    FOR v_cost IN
      ...
      ORDER BY item_order
    LOOP
      -- Determine supplier account based on partner
      v_supplier_account := '4350'; -- default domestic
      ...
      -- Debit inventory account (zavisni trošak povećava vrednost zaliha)
      ...
      -- Credit supplier account (obaveza prema dobavljaču troška)
      v_item_order := v_item_order + 1;
      INSERT INTO journal_entry_items (
        journal_entry_id, company_id, account_code, item_order, description,
    ...
    IF v_total_markup > 0 THEN
      -- Debit inventory account for markup (roba ulazi po prodajnoj ceni)
      v_item_order := v_item_order + 1;
      INSERT INTO journal_entry_items (
        journal_entry_id, company_id, account_code, item_order, description,
        debit_amount, credit_amount, document_date
      ...
      -- Credit RUC account (1329 - razlika u ceni robe)
      v_item_order := v_item_order + 1;
      INSERT INTO journal_entry_items (
        journal_entry_id, company_id, account_code, item_order, description,
        debit_amount, credit_amount, document_date
...
-- Create unpost function for consistency
CREATE OR REPLACE FUNCTION public.unpost_purchase_price_calculation(
  _calculation_id uuid,
  _user_id uuid
) RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Revert article selling prices (restore from previous purchase_price or 0)
  -- Note: We reset to the article's current purchase_price as a fallback
  FOR v_calc_item IN
