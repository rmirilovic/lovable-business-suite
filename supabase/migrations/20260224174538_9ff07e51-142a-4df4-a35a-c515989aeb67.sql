...
-- Update post_delivery_note: also set linked delivery order to 'shipped' and delete its reservations
...
BEGIN
  ...
  -- If delivery note was created from a delivery order, update its status and remove reservations
  ...
  IF v_order_id IS NOT NULL THEN
    ...
    WHERE document_type = 'delivery_order'
      AND document_id = v_order_id::text;
  END IF;
...
-- Update unpost_delivery_note: restore delivery order to 'reserved' and re-create reservations
...
BEGIN
  ...
  -- If delivery note was created from a delivery order, restore it
  ...
  IF v_order.id IS NOT NULL THEN
    ...
    LOOP
      ...
      SELECT
        ...
        now()::date, 'delivery_order', v_order.id::text, v_order.order_number,
