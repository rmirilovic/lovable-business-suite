...
-- Fix post_received_credit_note:
-- 1. Prefix "PKO" (not "PKO-")
-- 2. Non-deductible VAT goes to expense account (line_total instead of line_subtotal)
-- 3. Balance validation before completing
...
BEGIN
  ...
  -- Generate PKO prefixed entry number (no dash)
  ...
  LOOP
    -- If VAT is NOT deductible, expense gets full amount (subtotal + VAT)
    -- If VAT IS deductible, expense gets only subtotal
  ...
  -- Balance validation - prevent unbalanced journal entries
...
-- Add a generic balance validation trigger for ALL journal entries
-- This will prevent any unbalanced entry from being committed
...
BEGIN
  IF NEW.status = 'posted' THEN
    ...
    FROM journal_entry_items WHERE journal_entry_id = NEW.id;

    IF _total_debit != _total_credit THEN
      RAISE EXCEPTION 'Nalog % nije uravnotežen! Duguje: %, Potražuje: %. Knjiženje nije moguće.', NEW.entry_number, _total_debit, _total_credit;
    END IF;
  END IF;
  RETURN NEW;
END;
