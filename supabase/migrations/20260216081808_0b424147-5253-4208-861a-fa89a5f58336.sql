...
BEGIN
  RETURN QUERY
  WITH movements AS (
    -- Prijemnice (standalone, not linked to calculation)
    SELECT
      gr.warehouse_id,
    ...
    -- Kalkulacije (warehouse via goods_receipt)
    SELECT
      gr.warehouse_id,
    ...
    -- Nivelacije (using value_difference)
    SELECT
      pa.warehouse_id,
      'Nivelacija'::TEXT AS doc_type,
      SUM(CASE WHEN pai.value_difference > 0 THEN pai.value_difference ELSE 0 END) AS debit_val,
      SUM(CASE WHEN pai.value_difference < 0 THEN ABS(pai.value_difference) ELSE 0 END) AS credit_val
