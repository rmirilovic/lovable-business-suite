...
BEGIN
  ...
  -- Prihodi grupisani po SVK i poreskoj stopi
  FOR _svk_rec IN
    SELECT
  ...
  LOOP
    IF _is_foreign THEN
      _income_account := CASE _svk_rec.svk_code
        WHEN '1' THEN '6050'  -- Prihodi od prodaje robe na ino tržištu
        WHEN '9' THEN '6150'  -- Prihodi od prodaje GP na ino tržištu
        WHEN '0' THEN '6152'  -- Prihodi od vršenja usluga na ino tržištu
        ELSE _default_income_account
      END;
    ELSE
      _income_account := _default_income_account;
    END IF;
