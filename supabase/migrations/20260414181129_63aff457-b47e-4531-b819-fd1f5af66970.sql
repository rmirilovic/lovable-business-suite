...
BEGIN
  ...
  all_movements AS (
    -- Goods receipts (in)
    SELECT gri.article_id, gri.variant_id, gr.receipt_date::DATE AS movement_date, gri.quantity AS in_qty, 0::NUMERIC AS out_qty
    ...
    SELECT dni2.article_id, dni2.variant_id, dn2.delivery_date::DATE, 0::NUMERIC, dni2.quantity
    ...
    UNION ALL
    -- Inventory counts - surplus (in)
    SELECT ici.article_id, ici.variant_id, ic.count_date::DATE, ici.surplus_qty, 0::NUMERIC
    ...
    UNION ALL
    -- Inventory counts - deficit (out)
    SELECT ici.article_id, ici.variant_id, ic.count_date::DATE, 0::NUMERIC, ici.deficit_qty
    FROM inventory_count_items ici
    JOIN inventory_counts ic ON ic.id = ici.inventory_count_id
    WHERE ic.warehouse_id = v_warehouse_id AND ic.company_id = v_company_id AND ic.status = 'posted'
    ...
    UNION ALL
    -- Material requisitions / trebovanja (out)
    SELECT mri.article_id, mri.variant_id, mr.requisition_date::DATE, 0::NUMERIC, mri.quantity
    FROM material_requisition_items mri
    JOIN material_requisitions mr ON mr.id = mri.requisition_id
    WHERE mr.warehouse_id = v_warehouse_id AND mr.company_id = v_company_id AND mr.status = 'posted'
    AND mri.article_id IN (SELECT di.article_id FROM dn_items di)

    UNION ALL
    -- Article swaps (out - article_1)
    SELECT asw.article_1_id, NULL::UUID, asw.swap_date::DATE, 0::NUMERIC, asw.quantity_1
    FROM article_swaps asw
    WHERE asw.company_id = v_company_id AND asw.warehouse_id = v_warehouse_id AND asw.status = 'posted'
    AND asw.article_1_id IN (SELECT di.article_id FROM dn_items di)

    UNION ALL
    -- Article swaps (in - article_2)
    ...
    SELECT rwi.article_id, rwi.variant_id, rwo.order_date::DATE, 0::NUMERIC, rwi.quantity
    ...
    UNION ALL
    -- Reprocessing work order materials (out)
    SELECT rwm.article_id, rwm.variant_id, rwo.order_date::DATE, 0::NUMERIC, rwm.quantity
    FROM reprocessing_wo_materials rwm
    JOIN reprocessing_work_orders rwo ON rwo.id = rwm.work_order_id
    WHERE rwo.company_id = v_company_id AND rwo.status = 'closed'
    AND COALESCE(rwm.warehouse_id, rwo.warehouse_id) = v_warehouse_id
    AND rwm.article_id IN (SELECT di.article_id FROM dn_items di)

    UNION ALL
    -- Variant swaps (out - source)
    SELECT vs.article_id, vs.source_variant_id, vs.swap_date::DATE, 0::NUMERIC, vs.quantity
    FROM variant_swaps vs
    WHERE vs.company_id = v_company_id AND vs.warehouse_id = v_warehouse_id AND vs.status = 'posted'
    AND vs.article_id IN (SELECT di.article_id FROM dn_items di)

    UNION ALL
    -- Variant swaps (in - target)
    SELECT vs.article_id, vs.target_variant_id, vs.swap_date::DATE, vs.quantity, 0::NUMERIC
    FROM variant_swaps vs
    WHERE vs.company_id = v_company_id AND vs.warehouse_id = v_warehouse_id AND vs.status = 'posted'
    AND vs.article_id IN (SELECT di.article_id FROM dn_items di)
