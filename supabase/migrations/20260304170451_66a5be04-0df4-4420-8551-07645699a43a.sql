...
-- Add cost_center_code column to bank_statement_items
ALTER TABLE public.bank_statement_items
ADD COLUMN cost_center_code TEXT DEFAULT NULL;

-- Update post_bank_statement function to pass cost_center_code from items
...
BEGIN
  ...
  LOOP
    ...
    -- Each item books to its account code with its own analytics, debit and credit as entered
  ...
  -- Contra entries on 2410 with analytics = bank account code (TR)
  -- 2410 debits what the statement credits (uplata na TR)
  -- 2410 credits what the statement debits (isplata sa TR)
  _item_idx := _item_idx + 1;
  
  IF _total_credit > 0 THEN
    -- Debit 2410 for total credits (uplate na TR)
    INSERT INTO journal_entry_items (
      journal_entry_id, company_id, account_code, item_order,
      description, debit_amount, credit_amount, partner_id,
      cost_center_code, document_date
      ...
      'Žiro račun - uplate', _total_credit, 0, NULL,
  ...
  IF _total_debit > 0 THEN
    _item_idx := _item_idx + 1;
    -- Credit 2410 for total debits (isplate sa TR)
    INSERT INTO journal_entry_items (
      journal_entry_id, company_id, account_code, item_order,
      description, debit_amount, credit_amount, partner_id,
      cost_center_code, document_date
      ...
      _je_id, _doc.company_id, '2410', _item_idx,
      'Žiro račun - isplate', 0, _total_debit, NULL,
      _ba_code, _doc.statement_date
    );
  END IF;
