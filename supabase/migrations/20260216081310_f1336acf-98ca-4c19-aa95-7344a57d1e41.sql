...
BEGIN
  RETURN QUERY
  WITH movements AS (
    -- Goods receipts (Prijemnice) → debit
    SELECT
      gr.warehouse_id,
    ...
    -- Calculations (Kalkulacije) → debit (selling value)
    SELECT
      ppc.warehouse_id,
    ...
    GROUP BY ppc.warehouse_id

    UNION ALL

    -- Price adjustments / Nivelacije → debit or credit depending on sign
    SELECT
      pa.warehouse_id,
    ...
    -- Inventory counts - surplus → debit
    SELECT
      ic.warehouse_id,
    ...
    -- Inventory counts - deficit → credit
    SELECT
      ic.warehouse_id,
    ...
    -- Inter-warehouse transfers - source (credit / outgoing)
    SELECT
    ...
    -- Inter-warehouse transfers - destination (debit / incoming)
    SELECT
    ...
    -- Article swaps → debit (article 2 in) and credit (article 1 out)
    SELECT
      asw.warehouse_id,
    ...
    -- Delivery notes (Otpremnice) → credit (outgoing from warehouse)
    SELECT
      dn.warehouse_id,
