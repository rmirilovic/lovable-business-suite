-- Add document_date column to journal_entry_items for per-item valuta tracking
ALTER TABLE public.journal_entry_items
ADD COLUMN document_date DATE;

-- Update existing items to use the journal entry's document_date as default
UPDATE public.journal_entry_items jei
SET document_date = je.document_date
FROM public.journal_entries je
WHERE jei.journal_entry_id = je.id;

-- Drop existing functions first
DROP FUNCTION IF EXISTS public.post_goods_purchase_invoice(uuid, uuid);
DROP FUNCTION IF EXISTS public.post_service_purchase_invoice(uuid, uuid);

-- Recreate post_goods_purchase_invoice with correct document_date per item
CREATE FUNCTION public.post_goods_purchase_invoice(_invoice_id uuid, _user_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_supplier_account_code text := '4320';
...
-- Recreate post_service_purchase_invoice with correct document_date per item
CREATE FUNCTION public.post_service_purchase_invoice(_invoice_id uuid, _user_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
