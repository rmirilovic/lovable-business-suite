...
RETURNS TABLE(
  ...
  opening_qty NUMERIC, in_qty NUMERIC, out_qty NUMERIC, turnover NUMERIC, closing_qty NUMERIC
)
...
BEGIN
  ...
  WITH movements AS (
    ...
    FROM article_swaps asw WHERE asw.company_id = p_company_id AND asw.warehouse_id = p_warehouse_id AND asw.status = 'posted'
    UNION ALL
    SELECT pdni.article_id, pdni.qty_total, pdn.delivery_date, 'in'
    ...
    UNION ALL
    SELECT rdni.article_id, rdni.qty_total, rdn.delivery_date::DATE, 'in'
