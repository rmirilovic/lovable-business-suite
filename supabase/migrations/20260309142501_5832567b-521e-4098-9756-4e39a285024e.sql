...
BEGIN
  ...
  -- POPDV section 8a field 005 (smanjenje prethodnog poreza)
  ...
  IF _total_subtotal > 0 THEN
    INSERT INTO popdv_report_detail_rows (
      ...
      document_type_number, partner_info, values, item_order
    ) VALUES (
      _popdv_report_id, _doc.company_id, '8a', '8a.5',
      'PKO ' || _doc.internal_number,
      COALESCE(_doc.supplier_name, ''),
...
BEGIN
  ...
  DELETE FROM popdv_report_detail_rows WHERE report_id IN (
    SELECT id FROM popdv_reports WHERE company_id = _doc.company_id
  ) AND document_type_number LIKE 'PKO ' || _doc.internal_number || '%';
