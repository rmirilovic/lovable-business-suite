...
CREATE OR REPLACE FUNCTION get_article_warehouse_card(
  p_company_id UUID,
  p_warehouse_id UUID,
  p_article_id UUID,
...
BEGIN
  ...
  -- Opening balance: aggregate all movements before p_date_from
  opening AS (
    SELECT
  ...
  -- Return opening row (only if p_date_from is set and there's opening data) then period rows
