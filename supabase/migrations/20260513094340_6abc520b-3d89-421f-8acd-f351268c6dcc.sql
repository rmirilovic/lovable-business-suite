...
BEGIN
  ...
  WITH all_movements AS (
    ...
    UNION ALL
    SELECT vs.swap_date, 'Zamena varij. storno'::TEXT, vs.swap_number, ''::TEXT, -vs.quantity, 0::NUMERIC, vs.unit_price, -COALESCE(vs.swap_value, 0), 0::NUMERIC
    FROM variant_swaps vs
    WHERE vs.company_id = p_company_id AND vs.warehouse_id = p_warehouse_id AND vs.article_id = p_article_id AND vs.status = 'posted'
      AND (p_variant_id IS NULL OR vs.source_variant_id = p_variant_id)
    UNION ALL
    SELECT vs.swap_date, 'Zamena varij. ulaz'::TEXT, vs.swap_number, ''::TEXT, vs.quantity, 0::NUMERIC, vs.unit_price, COALESCE(vs.swap_value, 0), 0::NUMERIC
    FROM variant_swaps vs
    WHERE vs.company_id = p_company_id AND vs.warehouse_id = p_warehouse_id AND vs.article_id = p_article_id AND vs.status = 'posted'
      AND (p_variant_id IS NULL OR vs.target_variant_id = p_variant_id)
  ),
  opening AS (
    SELECT
