...
-- Fix: Use net price (after discount) when creating goods receipt items from purchase invoice
...
BEGIN
  ...
  -- Get warehouse type for stock account determination
  SELECT warehouse_type INTO v_warehouse_type FROM warehouses WHERE id = v_invoice.warehouse_id;
  
  -- Determine stock account based on warehouse type
  v_stock_account := CASE v_warehouse_type
  ...
  INSERT INTO journal_entries (
    company_id, year_id, entry_number, entry_date, document_number,
  ...
  -- Entry 1: Debit Stock account (materials/goods)
