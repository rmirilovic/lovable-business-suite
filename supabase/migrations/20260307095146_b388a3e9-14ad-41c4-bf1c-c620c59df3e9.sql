...
-- Add advance_invoice_id to invoices table
ALTER TABLE public.invoices 
ADD COLUMN advance_invoice_id uuid REFERENCES public.advance_invoices(id) ON DELETE SET NULL;

-- Create or replace the post_invoice function with advance handling
...
BEGIN
  ...
  IF _invoice.advance_invoice_id IS NOT NULL THEN
    ...
    IF FOUND AND _advance_doc.status = 'posted' THEN
      -- Reduce receivable by advance total amount
      _item_order := _item_order + 1;
      INSERT INTO journal_entry_items (
        journal_entry_id, company_id, account_code, item_order, description,
        debit_amount, credit_amount, partner_id
      ...
      -- Reverse advance PDV per VAT rate
      FOR _adv_vat_rec IN
      ...
      LOOP
        -- Duguje 4720/4730 (reverse the advance PDV credit)
        _item_order := _item_order + 1;
        INSERT INTO journal_entry_items (
          journal_entry_id, company_id, account_code, item_order, description,
          debit_amount, credit_amount
      ...
      -- Potražuje 4300 (reverse the advance PDV debit)
  ...
  -- 6. POPDV integration - regular invoice entry (3.2)
  ...
  IF FOUND THEN
    ...
    -- POPDV advance reversal entry in 3.9 (negative values)
    IF _invoice.advance_invoice_id IS NOT NULL AND (_adv_popdv_base_20 > 0 OR _adv_popdv_vat_20 > 0 OR _adv_popdv_base_10 > 0 OR _adv_popdv_vat_10 > 0) THEN
      SELECT COALESCE(MAX(item_order), 0) + 1 INTO _popdv_next_order
      FROM popdv_report_detail_rows
      WHERE report_id = _popdv_report.id AND section = '3' AND row_code = '3.9';
      ...
      INSERT INTO popdv_report_detail_rows (
        ...
        _popdv_report.id, _invoice.company_id, '3', '3.9',
        _invoice.invoice_date::date,
