...
-- Fix post_inventory_count to include cost_center_code on inventory account items
-- Fix unpost_inventory_count to use SECURITY DEFINER to bypass RLS
...
BEGIN
  ...
  IF v_surplus_total > 0 OR v_deficit_total > 0 THEN
    ...
    -- Surplus: Debit inventory account (with warehouse as cost_center), Credit 6790
    IF v_surplus_total > 0 THEN
      v_item_order := v_item_order + 1;
      INSERT INTO journal_entry_items (journal_entry_id, company_id, account_code, item_order,
        description, debit_amount, credit_amount, cost_center_code)
      VALUES (v_je_id, v_ic.company_id, v_inventory_account, v_item_order,
        'Višak po popisu ' || v_ic.count_number, v_surplus_total, 0, v_ic.warehouse_code);
    ...
    -- Deficit: Debit 5790, Credit inventory account (with warehouse as cost_center)
    IF v_deficit_total > 0 THEN
      ...
      INSERT INTO journal_entry_items (journal_entry_id, company_id, account_code, item_order,
        description, debit_amount, credit_amount, cost_center_code)
      VALUES (v_je_id, v_ic.company_id, v_inventory_account, v_item_order,
        'Manjak po popisu ' || v_ic.count_number, 0, v_deficit_total, v_ic.warehouse_code);
    END IF;
...
-- Fix unpost to use SECURITY DEFINER so it can delete posted journal entries
