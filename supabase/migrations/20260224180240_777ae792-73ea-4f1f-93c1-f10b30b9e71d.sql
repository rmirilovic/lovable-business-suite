...
-- Fix both functions: document_type='Nalog za isporuku', created_by is UUID not text
...
BEGIN
  ...
  IF v_order_id IS NOT NULL THEN
    ...
    WHERE document_type = 'Nalog za isporuku'
  ...
  IF v_order.id IS NOT NULL THEN
    ...
    LOOP
      ...
      SELECT
        ...
        now()::date, 'Nalog za isporuku', v_order.id, v_order.order_number,
        v_order.partner_id, p.code, p.name,
        'Vraćeno poništavanjem otpremnice', _user_id, ''
      FROM partners p
