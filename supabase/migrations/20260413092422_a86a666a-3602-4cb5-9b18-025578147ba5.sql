...
BEGIN
  ...
  WITH all_movements AS (
    ...
    UNION ALL
    -- Inter-warehouse transfers out (source)
    ...
    UNION ALL
    -- Inter-warehouse transfers in (destination)
    ...
    WHERE iwt.company_id = p_company_id AND iwt.destination_warehouse_id = p_warehouse_id AND iti.article_id = p_article_id AND iwt.status = 'posted'
      AND (p_variant_id IS NULL OR iti.variant_id = p_variant_id)
    UNION ALL
    -- Article swaps (storno) - only when no variant filter (swaps don't have variants)
    ...
    UNION ALL
    -- Article swaps (add) - only when no variant filter
    ...
    UNION ALL
    -- Reprocessing work orders - input (GP out)
    ...
    UNION ALL
    -- Reprocessing work orders - materials
